...
describe('formValidationSchema', () => {
  ...
  it('should require a name', async () => {
    await expect(
      formValidationSchema.validateAt('name', { name: '' })
    ).rejects.toThrow('Name is required');
  });

  it('should require name to start with an uppercase letter', async () => {
    await expect(
      formValidationSchema.validateAt('name', { name: 'john' })
    ).rejects.toThrow('Name must start with an uppercase letter');
  });
  ...
  it('should require an email', async () => {
    await expect(
      formValidationSchema.validateAt('email', { email: '' })
    ).rejects.toThrow('Email is required');
  });

  it('should require a valid email format', async () => {
    await expect(
      formValidationSchema.validateAt('email', { email: 'invalid-email' })
    ).rejects.toThrow('Invalid email format');
  });

  it('should accept a valid email', async () => {
    await expect(
      formValidationSchema.validateAt('email', { email: 'test@example.com' })
  ...
  it('should require a password', async () => {
    await expect(
      formValidationSchema.validateAt('password', { password: '' })
    ).rejects.toThrow('Password is required');
  });

  it('should require password to be at least 8 characters', async () => {
    await expect(
      formValidationSchema.validateAt('password', { password: 'short' })
    ).rejects.toThrow('Password must be at least 8 characters');
  });

  it('should require password to have a number', async () => {
    await expect(
      formValidationSchema.validateAt('password', {
  ...
  it('should accept a valid password', async () => {
    await expect(
      formValidationSchema.validateAt('password', { password: 'Password123!' })
  ...
  it('should require a confirm password', async () => {
    await expect(
      formValidationSchema.validateAt('confirmPassword', {
  ...
  it('should require a country', async () => {
    await expect(
      formValidationSchema.validateAt('country', { country: '' })
    ).rejects.toThrow('Country is required');
  });

  it('should accept a valid country', async () => {
    await expect(
      formValidationSchema.validateAt('country', { country: 'Poland' })
